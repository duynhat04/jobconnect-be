...
public class JobApplicationService {
    ...
    public JobApplication applyWithNewCV(String userEmail, Long jobId, MultipartFile cvFile, String coverLetter) throws Exception {
        if (cvFile == null || cvFile.isEmpty()) {
            throw new RuntimeException("Vui lòng chọn file CV để ứng tuyển!");
        }
    ...
    // LUỒNG 3: API cũ nộp CV, dùng lại logic upload CV mới
    @Transactional
    ...
    // API cũ: NTD xem tất cả hồ sơ ứng tuyển của công ty
    @Transactional(readOnly = true)
    ...
    // API mới: NTD xem ứng viên theo từng job, tự chấm điểm phù hợp với yêu cầu job
    @Transactional(readOnly = true)
    public Page<ApplicationCandidateDto> searchCandidatesForJob(
        ...
        List<ApplicationCandidateDto> pageContent =
